...
export const startTraining=createServerFn({method:"POST"}).inputValidator((x:{token:string;scenarioId:string})=>x).handler(async({data})=>{
...
export const trainingReply=createServerFn({method:"POST"}).inputValidator((x:{token:string;sessionId:string;message:string})=>({...x,message:String(x.message).slice(0,1200)})).handler(async({data})=>{
  const {supabaseAdmin,user}=await staffContext(data.token);
...
export const completeTraining=createServerFn({method:"POST"}).inputValidator((x:{token:string;sessionId:string})=>x).handler(async({data})=>{
  const {supabaseAdmin,user}=await staffContext(data.token);
...
export const trainingHistory=createServerFn({method:"POST"}).inputValidator((x:{token:string})=>x).handler(async({data})=>{
  const {supabaseAdmin,user}=await staffContext(data.token);
