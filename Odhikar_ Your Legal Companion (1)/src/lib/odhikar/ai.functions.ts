...
export const transcribeAudio = createServerFn({ method: "POST" })
  .inputValidator((input: { base64: string; mimeType: string }) => {
    if (typeof input?.base64 !== "string" || input.base64.length < 100) {
      throw new Error("Recording is empty or too short");
    }
    return { base64: input.base64, mimeType: input.mimeType || "audio/webm" };
  })
...
export const analyseNarrative = createServerFn({ method: "POST" })
  .inputValidator((input: { transcript: string; answers?: Record<string, string> }) => ({
