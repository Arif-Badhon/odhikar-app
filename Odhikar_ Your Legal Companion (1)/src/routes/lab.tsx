...
export const Route = createFileRoute("/lab")({
  head: () => ({
    meta: [
      { title: "Test Scenario Lab — Odhikar" },
      {
        name: "description",
        content:
          "Developer test harness: run seeded Odhikar narratives through the live safety, extraction and classification pipeline.",
      },
      { property: "og:title", content: "Test Scenario Lab — Odhikar" },
      {
        property: "og:description",
        content: "Seeded narratives executed against the real analysis services, never the client path.",
      },
      ...
      <header className="border-b border-border bg-primary text-primary-foreground">
        <div className="mx-auto flex max-w-6xl items-center gap-3 px-5 py-4">
          ...
          <div className="ml-auto flex gap-4 text-sm">
            ...
            <Link to="/paralegal" className="opacity-80 hover:opacity-100">
              Dashboard
            </Link>
          </div>
        </div>
                ...
                <div className="mt-auto flex gap-2 pt-5">
                  ...
                  {st?.caseId ? (
                    <Button asChild size="sm" variant="outline">
                      <Link to="/paralegal/$caseId" params={{ caseId: st.caseId }}>
                        Open case
                      </Link>
                    </Button>
                  ) : null}
